...
Original file is located at
    https://colab.research.google.com/drive/1EP_3I41ee8RxrDUaZp75hnCtuZxJqaqf
"""
...
df['Humidity (%)'].plot(kind='line', figsize=(20, 6))

# separate into 2 copies (training | testing)
msk = (df['Humidity (%)'].index < len(df)-36)
...
df_test['Humidity (%)'].plot(figsize = (10,5))
...
acf_original = plot_acf(df_train['Humidity (%)'])
pacf_original = plot_pacf(df_train['Humidity (%)'])

"""Method #3: ADF Test"""

from statsmodels.tsa.stattools import adfuller

adf_test = adfuller(df_train['Humidity (%)'])
...
stepwise_fit = auto_arima(df_train['Humidity (%)'], trace = True, supress_warnings = True, seasonal = True, m = 12)
...
model = ARIMA(df_train['Humidity (%)'], order = (p,d,q), seasonal_order=(P,D,Q,m))
model_fit = model.fit()
...
merged_data = pd.concat([df['Humidity (%)'], forecast_test], axis=1)
...
df_test['Humidity (%)'] = np.exp(df_test['Humidity (%)'])
forecast_test = np.exp(forecast_test)

# Perform the paired T-test
df = pd.read_csv('/content/drive/MyDrive/Adamson U/Courses/THESIS/Model/csv/weather data.csv')
df_t_test = pd.concat([df['Humidity (%)'].iloc[396:], forecast_test], axis=1)

t_statistic, p_value = stats.ttest_rel(df_t_test['Humidity (%)'], df_t_test['prediction'])

print("T-Statistic:", t_statistic)
print("P-Value:", p_value)

# MANUAL ARIMA
mae = mean_absolute_error(df_test['Humidity (%)'], forecast_test)
mape = mean_absolute_percentage_error (df_test['Humidity (%)'], forecast_test)
rmse = np.sqrt(mean_squared_error(df_test['Humidity (%)'], forecast_test))
...
model2 = ARIMA(df['Humidity (%)'], order = (p,d,q), seasonal_order=(P,D,Q,m))
model2 = model2.fit()
model2

pred = model2.predict(start = len(df['Humidity (%)']), end = len(df['Humidity (%)'])+11, type = 'levels').rename('SARIMA Predictions')
print(pred)
